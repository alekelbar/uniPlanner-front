...
export default function EditDeliverableDialog({
  onClose,
  open,
}: EditDeliverableDialogProps): JSX.Element {
  ...
  const selectedDeadline = parseISO(
    selected ? selected.deadline.toString() : new Date().toString()
  );
  ...
  useEffect(() => {
    ...
    if (selected) {
      ...
      formik.setFieldValue(
        "deadline",
        format(selectedDeadline, `yyyy-MM-dd'T'HH:mm`)
      );
  ...
  return (
    <>
      <Dialog
        ...
        open={open}
      >
        <DialogTitle>Actualización de Entrega</DialogTitle>
        <DialogContent>
          {!selectedDeliverable ? (
            <Typography>
              Parece que no se encuentra ningún entregable seleccionado
            </Typography>
          ) : (
            <Stack
              component={"form"}
